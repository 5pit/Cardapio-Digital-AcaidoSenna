const CELULAR_EMPRESA = '5532998083592';
...
buscarCep: () => {
    ...
    // Se o CEP for informado, faz a busca
    if (cep != '') {
    ...
    // Se o CEP não for informado, permite preenchimento manual
    else {
        cardapio.metodos.mensagem('Preencha as informações manualmente.');
        $('#txtEndereco').focus();
    }
},
...
mascaraCep: (event) => {
    ...
    // Executa a busca de CEP automaticamente se o campo estiver completo
    ...
    lojaAbertaOuFechada:() => {
        ...
        if((hora < LOJA_ABRE || hora >= LOJA_FECHA) || dia == 1 ){
            ...
            cardapio.metodos.mensagem(`Abrimos (Seg à Dom) às ${LOJA_ABRE.toFixed(2).replace(".", ":")} hrs.`, cor="red", tempo=15000);
